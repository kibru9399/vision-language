...
class kv_cache:
    ...
    def update(self,
        ...
        if len(self.key_cache) <= layer_idx:
            ...
            self.value_cache[layer_idx] = torch.cat([self.value_cache[layer_idx], key_states], dim=-2)
...
class GemmaRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids, seq_len=None):
        ...
        #inv_freq_expaned [batch_size, head_dim//2, 1]
        inv_freq_expaned = self.inv_freq[None, :, None].float().expand(position_ids.shape[0], -1, 1)
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            #freq: [batch_size, seq_len, head_dim//2]
            freqs = (inv_freq_expaned.float()@position_ids_expanded.float()).transpose(1, 2)
...
class GemmaAttention(nn.Module):
    ...
    def forward(
        ...
        cos, sin = self.rope_emb(value_states, position_ids, seq_len=None)
        ...
        if attn_out.size() !=  (bsz, self.num_heads, q_len, self.head_dim):
            raise ValueError(
                f"'attn_output' should be of size {(bsz, self.num_heads, q_len, self.head_dim)}\
                    , but is of size {attn_out.size()}"
            )
        attn_out = attn_out.transpose(2, 3).contiguous()
        attn_out = attn_out.view(bsz, q_len, -1)
...
class PaliGemmaForConditionalGeneration(nn.Module):
    ...
    def _merge_input_ids_with_image_features(
            self, image_features: torch.Tensor, input_embeds: torch.Tensor, input_ids: torch.Tensor, 
            attention_mask: torch.Tensor, kv_cache: KVCache = None
    ):
        ...
        dtype, device = input_embeds.dtype, input_embeds.device
        min_dtpe = torch.finfo(dtype).min
        ...
        if kv_cache is not None and kv_cache.num_items() > 0:
            position_ids = attention_mask.cumsum_(-1)[:, -1]
            if position_ids.dim() == 1:
                position_ids = position_ids.unsqueeze(0)
        else:
            position_ids = (position_ids.cumsum(-1)).masked_fill((attention_mask == 0), 1).to(device)
    ...
    def forward(
        ...
        input_embdeds = self.language_model.get_input_embeddings()(input_ids)
        selected_image_feature = self.vision_tower(pixel_values.to(input_embdeds.dtype))
        image_features = self.multi_model_projector(selected_image_feature)
        input_embdeds, attention_mask, position_ids = self._merge_input_ids_with_image_features(
            image_features, input_embdeds, attention_mask, kv_cache
        )
        outputs = self.language_model(
            attention_mask=attention_mask, 
            position_ids=position_ids, 
            input_embdeds=input_embdeds, 
            kv_cache=kv_cache
        )
        return outputs